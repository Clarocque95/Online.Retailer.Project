...
-- We see that some products have more than one product_id.
...
GROUP BY brand
                                                ...
                                                -- PROBLEM 4 --
-- PART 1
-- For each brand, find the following information: number of products sold, sum of total sales and the minimum, maximum, average of the sale price. 

-- We must use MIN, MAX, ROUND, AVG and SUM functions to solve this problem. 
-- I joined the transactions and brands tables and used GROUP BY for brand.
-- The final table shows that Adidas has more products than Nike and more revenue. Also, we see that Nike has nearly double the average sale price than Adidas. 
SELECT brand, 
...
-- I first need the revenue of a particular brand. Then, I use this result in a subquery and divide it by total revenue. 
...
SELECT
	...
	CONCAT(ROUND((SELECT SUM(sale_price) 
	              ...
	              ON t.product_id = b.product_id 
	              WHERE brand = 'Nike')
	/SUM(sale_price)*100, 2), '%') AS nike_portion_revnue
FROM transactions
...
-- I used ORDER BY clause to order the revenue in descending order and I used LIMIT clause to only obtain top 10 rows (top 10 products).
...
-- The table shows that the product with the highest revenue represents over 2% of the total revenue for Nike products and the next product is at 1.75%. 
SELECT brand, 
       product_name, 
...
-- A CASE statement must be used to achieve the desired result. I also joined the reviews, brands and transactions tables and ORDERED the results by total revenue. 
...
-- The results shows that most products have a description length between 200-299 and that the average is similar for each category. 
SELECT 
	CASE
...
-- Finally, it's critical to use the LOWER function in the WHERE clause. The LIKE operator is case sensitive, therefore 'SHOES' is different than 'Shoes'. 
...
-- This shows that about 2/3 of the revenue is from shoe sales. 
SELECT CONCAT(ROUND((SELECT SUM(sale_price) AS revenue_shoes
		FROM info i
        JOIN transactions t
        ON i.product_id = t.product_id
        WHERE LOWER(product_name) LIKE '%shoes%') / SUM(sale_price)*100, 2), '%') AS shoe_prodcuts_revenue_portion
FROM transactions
...
-- In there WHERE clause, I added a subquery that returns the max count for a specific brand. I used the UNION operator to join the result for Adidas with the result for Nike.
WITH CTE AS 
(
...
GROUP BY brand, sale_price, count
                                                ...
                                                -- PROBLEM 11 --
-- PART 1 
-- Which months have a total revenue over 45000$?

-- The DATE_PART function is used in this query to get the month and the year from the date column.
-- The function SUM was used to calculate the total revenue for every month.
-- I added a HAVING clause since it's impossible to use aggregate functions in a WHERE CLAUSE. This clause will filter out all months with less than 45000$ in revenue. 
...
-- To find the average per month for each year, we must divide the total yearly revenue by the number of months within that year.
...
-- The previous table showed that the average revenue for 2020 is quite low compared to 2018 and 2019. 
...
-- In this query, I filter out data from the month of April and only keep the data from January to March (3 months in total).
...
-- Also, this time I used a UNION operator so that we can see all data in one table.  
...
-- I used a case statement to replace the months from numbers to actual name of the month. This makes it easier to read and quickly which months have the highest sales. 
-- I exclude sales from 2020, since the sales stops in the month of April. If I had kept this data, it would have skewed the results for the months of Jan, Feb and March.
-- The data is aggregated by month and the total revenue is sorted in descending order.
-- We that that July has the highest total sales and that february has the lowest total sales. 
SELECT CASE 
       WHEN DATE_PART('month', date) = 1 THEN 'January' 
       WHEN DATE_PART('month', date) = 2 THEN 'Febuary' 
...
-- Finally the data is aggregated by season and brand and sorted total revenue un descending order. 
SELECT CASE 
...
-- We need the window functions LAG() to solve this problem.
...
-- The query is like the previous query, but it's more complicated since we can't combine AVG and SUM functions together (Can't use AVG(SUM(sale_price)))
...
-- PART 2
-- Create a query that will product a table showing the daily moving average for the month of May in 2018.
